...
add_action( 'wp_ajax_portfolio_ajax_load_more_vc', 'tmreviews_portfolio_ajax_load_more_vc');
add_action( 'wp_ajax_nopriv_portfolio_ajax_load_more_vc', 'tmreviews_portfolio_ajax_load_more_vc');
function tmreviews_portfolio_ajax_load_more_vc() {
    ...
    if ( $posts->have_posts() ) :  while ( $posts->have_posts() ): $posts->the_post(); $i++;?>
        <article <?php post_class(esc_attr( trim( $portfolio_item_css_class ) )) ?> id="post-<?php the_ID() ?>" data-post-id="<?php the_ID() ?>">
            <div class="entry-content">
                <?php
                if(! empty($portfolio_style) and $portfolio_style =='portfolio-masonry-style'){
                    if($i == '2'){
                        echo get_the_post_thumbnail($posts->ID, 'gazek_size_570x845_crop');
                    }else{
                        echo get_the_post_thumbnail($posts->ID, 'gazek_size_570x400_crop');
                    }
                }else{
                    echo get_the_post_thumbnail($posts->ID, 'gazek_size_570x400_crop');
                }
                ...
                <div class="portfolio-mask-content">
                    <a class="portfolio-title-link fl-text-title-style" href="<?php esc_url(the_permalink()); ?>"><?php esc_attr(the_title()); ?></a>
