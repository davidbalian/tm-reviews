...
$title = gazek_get_theme_mod ('places_title_search_results');
if(!isset($title) && $title == ''){
    $title = __('Reviews Search Results', 'tm-reviews');
}

$seacrh_title = gazek_get_theme_mod ('places_title_search');
$seacrh_title_js = '"'.$seacrh_title.'"';
if(!isset($seacrh_title) && $seacrh_title == ''){
    $seacrh_title = __('Filter By Keyword', 'tm-reviews');
}
...
if(isset($_GET['order']) && $_GET['order'] == 'rate_asc'){
    $order = 'ASC';
} elseif(isset($_GET['order']) && $_GET['order'] == 'rate_desc') {
    $order = 'DESC';
} elseif(isset($_GET['order']) && $_GET['order'] == 'date_desc') {
    $order = 'DESC';
} elseif(isset($_GET['order']) && $_GET['order'] == 'date_asc') {
    $order = 'ASC';
}
...
$tax_name = '';
if(isset($_GET[tmreviews_get_post_type() . '-category']) && $_GET[tmreviews_get_post_type() . '-category'] != 'all'){
...
if(isset($_GET['order']) && $_GET['order'] != ''){
    if($_GET['order'] == 'rate_desc' || $_GET['order'] == 'rate_asc'){


        if($_GET['order'] == 'rate_asc'){
            $args['order'] = 'ASC';
        }

        if($_GET['order'] == 'rate_desc'){
            $args['order'] = 'DESC';
        }

        $args['orderby'] =  'meta_value_num';
        $args['meta_key']  = 'tmreviews_post_average';
        $args['meta_type']  = 'NUMERIC';
    }


    if($_GET['order'] == 'date_desc' || $_GET['order'] == 'date_asc'){

        if($_GET['order'] == 'date_asc'){
            $args['order'] = 'ASC';
        }

        if($_GET['order'] == 'date_desc'){
            $args['order'] = 'DESC';
        }
        $args['orderby'] =  'date';
...
<div class="fl-category-places-header jarallax">
    ...
    <div class="fl-places-categories-header-meta container">
        ...
        <?php } else { ?>
            <div class="fl-places-header-bottom">
                <span class="fl-places-in-category"><?php echo __('In All Categories', 'tm-reviews'); ?></span>
            </div>
        <?php }?>
    </div>
</div>
            ...
            <form id="searchform" action="<?php echo esc_url($category_template, 'tm-reviews');?>" method="get">
               <div class="searchform-wrap-fl">
                ...
                <select class="places-order tmnice-select" name="order">
                    ...
                    <?php if(tmreviews_chech_if_exist_meta()){?>
        ...
        <div class="fl-category-single col-md-4">
            ...
            <div class="fl-category-single-middle">
                ...
                <div class="fl-places-average-cat">
                    <?php if(isset($categories_html) && $categories_html != ''){?>
            ...
            <div class="fl-category-single-bottom">
                ...
                <span class="fl-review-author-name"><a href="<?php echo esc_url($user_page_link) ?>?author=<?php echo $author->user_nicename;?>"><?php echo $author_name; ?></a></span>
