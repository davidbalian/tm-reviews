...
$disable_element = $vc_css = $responsive_css = $output = $after_output = $responsive_style = $custom_css_row = $tmreviews_gradient=$opacity_gr_bg ='';
...
if ( ! empty( $box_shadow_param ) and $box_shadow_param !='' ) {
    ...
    $tmreviews_helping_css_style[] = '.' . $box_shadow_html . ' {'. $box_shadow_param. '}';
}

if ( ! empty( $row_top ) and $row_top !='' ) {
    $row_custom_html = uniqid('fl-custom-html-row-').'-'.rand(100,9999);
    $css_classes[] = $row_custom_html;
    $tmreviews_helping_css_style[] = '.' . $row_custom_html . ' {top:'. $row_top. 'px;position:relative;}';
}
...
if ( ! empty( $tmreviews_overflow_type ) and ($tmreviews_overflow_type =='overflow-visible')) {
    $wrapper_attributes[]   = 'data-row-overflow-visible="true"';
}elseif ( ! empty( $tmreviews_overflow_type ) and ($tmreviews_overflow_type =='overflow-auto')){
    $wrapper_attributes[]   = 'data-row-overflow-visible-auto="true"';
}elseif ( ! empty( $tmreviews_overflow_type ) and ($tmreviews_overflow_type =='overflow-hidden')){
