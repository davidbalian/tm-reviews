...
function tmreviews_helping__addons_get_responsive_style( $responsive_css, $responsive_id ) {

    return TMReviews__Helping_Responsive_CSS_Style::get_css_style( $responsive_css, $responsive_id );
}

if( ! class_exists( 'TMReviews__Helping_Responsive_CSS_Style' ) ) {

    class TMReviews__Helping_Responsive_CSS_Style  {
        ...
        function __construct() {
            ...
            if ( function_exists( 'vc_add_shortcode_param' ) ) {
                vc_add_shortcode_param('tmreviews_responsive_css_editor',  array( $this, 'tmreviews_custom_responsive_css_settings_field' ), TMREVIEWS_THEME_HELPER_ROOT_DIR . '/assets/params/js/fl-param_responsive.js' );
            }
        }

        function tmreviews_custom_responsive_css_settings_field( $settings, $value ) {

            $output = $inner_output = '';

            $label  = isset( $settings['label'] ) ? $settings['label'] : esc_html__( 'Responsive Options', 'tm-reviews' );
            ...
            $i = 0;
                ...
                $output .= '<div class="fl-helping-responsive-title-wrapper">';
                    foreach( $devices as $device ) {
                        $title = ( $device == 'desktop' ) ? esc_html__( 'Mini Desktop', 'tm-reviews' ) : esc_html__( ucfirst( $device ), 'tm-reviews' );
                        $active = ( $i == 0 ) ? ' active' : '';
                        ...
                        $inner_output .= '<div class="fl-helping-main-responsive-wrapper '.$device.'-device'.$active.'">';
                            ...
                            $inner_output .= '<div class="fl-helping-inner-wrap">';
                                    ...
                                    if($i == '1'){
                                        $inner_output .= '<div class="fl-despot-heading">' . $devices_icons[$i] .esc_html__( 'Mini Destop Responsive Option', 'tm-reviews' ).'</div>';
                                        $inner_output .=  '<div class="vc_description vc_clearfix fl-responsive-description">'.esc_html__( 'Screen resolutions from 1199px to 991px', 'tm-reviews' ).'</div>';
                                    } elseif($i == '2') {
                                        $inner_output .= '<div class="fl-despot-heading">' . $devices_icons[$i] .esc_html__( 'Table Responsive Option', 'tm-reviews' ).'</div>';
                                        $inner_output .=  '<div class="vc_description vc_clearfix fl-responsive-description">'.esc_html__( 'Screen resolutions from 991px to 767px', 'tm-reviews' ).'</div>';
                                    } else {
                                        $inner_output .= '<div class="fl-despot-heading">' . '<i class="fa fa-mobile" aria-hidden="true"></i>' .esc_html__( 'Mobile Responsive Option', 'tm-reviews' ).'</div>';
                                        $inner_output .=  '<div class="vc_description vc_clearfix fl-responsive-description">'.esc_html__( 'Screen resolutions less than 767px', 'tm-reviews' ).'</div>';
                                    }
        ...
        function onionLayout( $prefix = '', $values = array() ) {
            ...
            if( isset( $values['background_position' . ( '' !== $prefix ? '_' . $prefix : '' ) . ''] ) ) {
                $output .= '   <label>' . esc_html__('Background position', 'tm-reviews') . '</label>'
                    ...
                    . '      <option value="" ' . selected('', $bg_position_value, false) . '>' . esc_html__('Default', 'tm-reviews') . '</option>'
                    . '          <option value="left-top" ' . selected('left-top', $bg_position_value, false) . '>' . esc_html__('Left Top', 'tm-reviews') . '</option>'
                    . '          <option value="left-center" ' . selected('left-center', $bg_position_value, false) . '>' . esc_html__('Left Middle', 'tm-reviews') . '</option>'
                    . '          <option value="left-bottom" ' . selected('left-bottom', $bg_position_value, false) . '>' . esc_html__('Left Bottom', 'tm-reviews') . '</option>'
                    . '          <option value="center-top" ' . selected('center-top', $bg_position_value, false) . '>' . esc_html__('Center Top', 'tm-reviews') . '</option>'
                    . '          <option value="center-center" ' . selected('center-center', $bg_position_value, false) . '>' . esc_html__('Center Middle', 'tm-reviews') . '</option>'
                    . '          <option value="center-bottom" ' . selected('center-bottom', $bg_position_value, false) . '>' . esc_html__('Center Bottom', 'tm-reviews') . '</option>'
                    . '          <option value="right-top" ' . selected('right-top', $bg_position_value, false) . '>' . esc_html__('Right Top', 'tm-reviews') . '</option>'
                    . '          <option value="right-center" ' . selected('right-center', $bg_position_value, false) . '>' . esc_html__('Right Middle', 'tm-reviews') . '</option>'
                    . '          <option value="right-bottom" ' . selected('right-bottom', $bg_position_value, false) . '>' . esc_html__('Right Bottom', 'tm-reviews') . '</option>'
                    . '    </select>'
                    . '  </div>';
            }
                $output .= '   <label>' . esc_html__('Hide image', 'tm-reviews') . '</label>'
                    . '  <div class="vc_hide-image">'
                    . '    <select name="hide_image' . ('' !== $prefix ? '_' . $prefix : '') . '" data-name="hide-image' . ('' !== $prefix ? '-' . $prefix : '') . '">'
                    . '      <option value="" ' . selected('', $hide_image_value, false) . '>' . esc_html__('No', 'tm-reviews') . '</option>'
                    . '      <option value="1" ' . selected('1', $hide_image_value, false) . '>' . esc_html__('Yes', 'tm-reviews') . '</option>'
                    . '    </select>'
                    . '  </div>';

            if( isset( $values['height' . ( '' !== $prefix ? '_' . $prefix : '' ) . ''] ) ) {
                $output .= '   <label>'.esc_html__( 'Minimum height', 'tm-reviews' ).'</label>'
                    . '  <div class="vc_height">'
                    . '    <input class="vc-height-input" type="text" name="height' . ( '' !== $prefix ? '_' . $prefix : '' ) . '" data-name="height' . ( '' !== $prefix ? '-' . $prefix : '' ) . '" value="'.$height_value.'" />'

                    . '  </div>';
                $output .='<span class="vc_description vc_clearfix">' . esc_html__('Example: 400px', 'tm-reviews') . '</span>';
            }
            if( isset( $values['width' . ( '' !== $prefix ? '_' . $prefix : '' ) . ''] ) ) {
                $output .= '   <label>'.esc_html__( 'Width', 'tm-reviews' ).'</label>'
                    . '  <div class="vc_width">'
        ...
        public static function get_css_style( $value, $id = '' ) {
            ...
            $values = TMReviews__Helping_Responsive_CSS_Style::get_responsive_css_values( $value );
...
new TMReviews__Helping_Responsive_CSS_Style;