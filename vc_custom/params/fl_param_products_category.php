<?php
    if ( function_exists( 'vc_add_shortcode_param' ) ) {
        ...
        vc_add_shortcode_param( 'tmreviews_custom_products_categories', 'tmreviews_custom_products_category_settings_field');

    }

if ( ! function_exists( 'tmreviews_custom_products_category' ) ) :
    function tmreviews_custom_products_category_settings_field( $settings, $value ) {
        ...
        if(!empty($value1)):
            ...
            $output .= '<option value="0" '.$selected_all.'>'.esc_html__('All Categories', 'tm-reviews').'</option>';
        else:
            $output .= '<option value="0" selected="selected">'.esc_html__('All Categories', 'tm-reviews').'</option>';
        endif;
